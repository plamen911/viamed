...
$cyrlat = new CyrLat;
$filename = short_text('Zdravni_Dosieta_'.$cyrlat->cyr2lat($results['firm_name']), 60);

require('phprtflite/rtfbegin.php');

$sect->writeText('Приложение № 6 към чл.11, ал.10', $times10, $alignRight);
$sect->addEmptyParagraph();
$sect->writeText('<b>ЗДРАВНИ ДОСИЕТА' . (($total > $limit) ? ' - Том '.($page + 1) : '') . '</b>', $times20, $alignCenter);
